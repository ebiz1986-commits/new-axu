...
async function evaluateBrains(currentPrice: number): Promise<BrainDecision> {
  ...
  if (emaFast > emaSlow) b2_confluence += 2; // uptrend alignment
  else b2_confluence += 2; // downtrend alignment (we aggregate absolute alignment)
...
function handleMarketTick() {
  ...
  if (state.activeTrade) {
    ...
    const isStale = state.simulationSpeed === "FAST"
       ? elapsedMinutes > 3.0 // fast mode limits
...
let ticksCountdown = 25; // 25 ticks closes a candle in fast mode
...
function startTickSimulation() {
  ...
  tickTimer = setInterval(() => {
    ...
    if (ticksCountdown <= 0) {
      ...
      ticksCountdown = state.simulationSpeed === "ULTRA" ? 15 : 25;
    }
